# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 21:33:01 2016
...
train_test_combined = pd.concat([train,test],axis=0)
train_test_proddesc = pd.merge(train_test_combined,product_description,on ="product_uid",how = "left" )
train_test_proddesc.head()

#concat attributes function
def concat_attributes(arr):
    names = arr["name"]
    value = arr["value"]
    joined = []
    for a,b in zip(names,value):
        joined.append(" ".join((a,b)))
    return " ".join(joined)
    
attributes["product_uid"] = attributes["product_uid"].astype(int)
attributes["value"] = attributes["value"].astype(str)

attri = attributes.groupby("product_uid").apply(concat_attributes)

attri = attri.reset_index(name="product_attributes")
trainset = pd.merge(train_test_proddesc,attri,on = "product_uid",how = "left")
trainset = pd.merge(trainset,df_brand,on = "product_uid",how = "left")

trainset["brand"] = trainset["brand"].astype(str)
trainset["product_attributes"] = trainset["product_attributes"].astype(str) 
 

#convert to lower
def conv_2_lower(arr):
    return arr.lower()
...
def stop_words(arr):
    ...
    stopwords_updated = stopwords_updated.update(("search","nan",'for', 'xbi', 'and', 'in', 'th','on','sku','with','what','from','that','less','er','ing'))
    return " ".join([w for w in arr.split() if not w in stopwords_updated])
...
def correct_spell(arr):
    for key,value in spell_check_dict.items():
        if key in arr:
            arr = arr.replace(key,value)
    return (arr)
...
trainset["search_term"] = trainset["search_term"].map(lambda x: conv_2_lower(x))
trainset["product_title"] = trainset["product_title"].map(lambda x: conv_2_lower(x))
trainset["product_description"] = trainset["product_description"].map(lambda x: conv_2_lower(x))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: conv_2_lower(x))
trainset["brand"] = trainset["brand"].map(lambda x: conv_2_lower(x))

trainset["search_term"] = trainset["search_term"].map(lambda x: correct_spell(x))
trainset["product_title"] = trainset["product_title"].map(lambda x: correct_spell(x))
trainset["product_description"] = trainset["product_description"].map(lambda x: correct_spell(x))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: correct_spell(x))
trainset["brand"] = trainset["brand"].map(lambda x: correct_spell(x))


#remove punctuations
regex = re.compile('[%s]' % re.escape(string.punctuation))
trainset["search_term"] = trainset["search_term"].map(lambda x: regex.sub("",x))
trainset["product_title"] = trainset["product_title"].map(lambda x: regex.sub("",x))
trainset["product_description"] = trainset["product_description"].map(lambda x: regex.sub("",x))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: regex.sub("",x))
trainset["brand"] = trainset["brand"].map(lambda x: regex.sub("",x))

#remove words less than 3 letters
trainset["search_term"] = trainset["search_term"].map(lambda x: " ".join([word for word in x.split() if (len(word)>=3)]))
trainset["product_title"] = trainset["product_title"].map(lambda x: " ".join([word for word in x.split() if (len(word)>=3)]))
trainset["product_description"] = trainset["product_description"].map(lambda x: " ".join([word for word in x.split() if (len(word)>=3)]))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: " ".join([word for word in x.split() if (len(word)>=3)]))

#remove stop words
sw = stopwords.words("english")
trainset["search_term"] = trainset["search_term"].map(lambda x: " ".join([word for word in x.split() if word not in sw]))
trainset["product_title"] = trainset["product_title"].map(lambda x: " ".join([word for word in x.split() if word not in sw]))
trainset["product_description"] = trainset["product_description"].map(lambda x: " ".join([word for word in x.split() if word not in sw]))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: " ".join([word for word in x.split() if word not in sw]))
...
#stem words
trainset["search_term"] = trainset["search_term"].map(lambda x: stem_word(x))
trainset["product_title"] = trainset["product_title"].map(lambda x: stem_word(x))
trainset["product_description"] = trainset["product_description"].map(lambda x: stem_word(x))
trainset["product_attributes"] = trainset["product_attributes"].map(lambda x: stem_word(x))
...
#feature creation
#text length
trainset["query_num_of_words"] = trainset["search_term"].map(lambda x: len(x.split())).astype(np.int64)
trainset["query_length of words"] = trainset["search_term"].map(lambda x: len(x)).astype(np.int64)
trainset["brand_num_of_words"] = trainset["brand"].map(lambda x: len(x.split())).astype(np.int64)
trainset["brand_length of words"] = trainset["brand"].map(lambda x: len(x)).astype(np.int64)
trainset["title_num_of_words"] = trainset["product_title"].map(lambda x: len(x.split())).astype(np.int64)
trainset["description_num_of_words"] = trainset["product_description"].map(lambda x: len(x.split())).astype(np.int64)
trainset["attributes_num_of_words"] = trainset["product_attributes"].map(lambda x: len(x.split())).astype(np.int64)


#words similar to search words
trainset["product_info"] = trainset["search_term"]+ "\t"+ trainset["product_title"] + "\t" + trainset["product_description"] + "\t" + trainset["product_attributes"] + "\t"+ trainset["brand"]
trainset["common_words_title"] = trainset["product_info"].map(lambda x: common_words(x.split("\t")[0],x.split("\t")[1]))
trainset["common_words_description"] = trainset["product_info"].map(lambda x: common_words(x.split("\t")[0],x.split("\t")[2]))     
trainset["common_words_attributes"] =  trainset["product_info"].map(lambda x: common_words(x.split("\t")[0],x.split("\t")[3]))   
trainset["common_words_brand"] =  trainset["product_info"].map(lambda x: common_words(x.split("\t")[0],x.split("\t")[4]))   
...
trainset['ratio_title'] = div0(trainset['common_words_title'],trainset['title_num_of_words'])
trainset['ratio_description'] = div0(trainset['common_words_description'],trainset['description_num_of_words'])
trainset['ratio_brand'] = div0(trainset['common_words_brand'],trainset['brand_num_of_words'])
trainset['ratio_attributes'] = div0(trainset['common_words_attributes'],trainset['attributes_num_of_words'])


#adding distance between words as feature
import distance
def dist(x,y):
    return (distance.levenshtein(x,y,normalized = True))
trainset["title_dist"] = [dist(x,y) for x,y in zip(trainset["search_term"],trainset["product_title"])]
trainset["desc_dist"] = [dist(x,y) for x,y in zip(trainset["search_term"],trainset["product_description"])]
trainset["attri_dist"] = [dist(x,y) for x,y in zip(trainset["search_term"],trainset["product_attributes"])]
trainset["brand_dist"] = [dist(x,y) for x,y in zip(trainset["search_term"],trainset["brand"])]

import nltk
from nltk.util import ngrams

trainset["words_desc"] = trainset["product_description"].map(lambda x: nltk.word_tokenize(x))
trainset["words_attri"] = trainset["product_attributes"].map(lambda x: nltk.word_tokenize(x))
trainset["words_brand"] = trainset["brand"].map(lambda x: nltk.word_tokenize(x))
trainset["words_title"] = trainset["product_title"].map(lambda x: nltk.word_tokenize(x))
trainset["words_search"] = trainset["search_term"].map(lambda x: nltk.word_tokenize(x))

trainset["n-gram_description"] = trainset["words_desc"].map(lambda x: list(ngrams(x,1)))
trainset["n-gram_attri"] = trainset["words_attri"].map(lambda x: list(ngrams(x,1)))
trainset["n-gram_brand"] = trainset["words_brand"].map(lambda x: list(ngrams(x,1)))
trainset["n-gram_title"] = trainset["words_title"].map(lambda x: list(ngrams(x,1)))
trainset["n-gram_search"] = trainset["words_search"].map(lambda x: list(ngrams(x,1)))
...
trainset = trainset.drop(['product_info','words_desc','words_attri','words_brand','words_title','words_search'],axis=1)
trainset = trainset.drop(['product_description','product_attributes'],axis=1)
trainset = trainset.drop(['search_term','product_title'],axis=1)
trainset = trainset.drop(['brand'],axis=1)

#trainset = trainset.drop(['ratio_title','ratio_description','ratio_brand','ratio_attributes'],axis=1)


plt.hist(trainset["query_num_of_words"],color = 'red')
plt.title('Distribution of number of words in query')
plt.xlabel('number of words')
plt.ylabel('frequency')

plt.hist(trainset["title_num_of_words"],color = "red")
plt.title('Distribution of number of words in title')
plt.xlabel('number of words')
plt.ylabel('frequency')

plt.hist(trainset["description_num_of_words"],color = "red")
plt.title('Distribution of number of words in description')
plt.xlabel('number of words')
plt.ylabel('frequency')

plt.hist(trainset["attributes_num_of_words"],color = "red")
plt.title('Distribution of number of words in attributes')
plt.xlabel('number of words')
plt.ylabel('frequency')
...
for ix in trainset.index:
    search_text += trainset["search_term"][ix] + " "

wordcloud = WordCloud().generate(search_text)
plt.figure()
...
title_text = ""
for ix in trainset.index:
    title_text += trainset["product_title"][ix] + " "

titlecloud = WordCloud().generate(title_text)
plt.figure()
...
desc_text = ""
for ix in trainset.index:
    desc_text += trainset["product_description"][ix] + " "

desccloud = WordCloud().generate(desc_text)
plt.figure()
...
attr_text = ""
for ix in trainset.index:
    attr_text += trainset["product_attributes"][ix] + " "
    
attricloud = WordCloud().generate(attr_text)
plt.figure()
plt.imshow(attricloud)
plt.axis("off")
plt.show()



df_train = trainset.iloc[:len(train)]
df_test = trainset.iloc[len(train):]

Y = df_train['relevance'].values
X = df_train.drop(['id','relevance'],axis=1)
testset = df_test.drop(['id','relevance'],axis=1)

for f in X.columns:
    if X[f].dtype=='object':
        print(f)
        lbl = preprocessing.LabelEncoder()
        lbl.fit(list(X[f].values) + list(testset[f].values))
        X[f] = lbl.transform(list(X[f].values))
        testset[f] = lbl.transform(list(testset[f].values))


gbr = GradientBoostingRegressor(loss='ls',random_state=2016)
parameters = {'learning_rate': [0.07,0.8,0.1], #when use hyperthread, xgboost may become slower
              'max_depth': [7],
              'n_estimators' : [100], 
              'max_features': ['auto']
              #'num_class':3,
              }
clf = BaggingRegressor(gbr, n_estimators=100, max_samples=0.1, random_state=25)
              


rfr = RandomForestRegressor()
rfr = RandomForestRegressor(n_estimators = 100,max_depth = 8, n_jobs = -1, random_state = 2016, verbose = 1)
clf = BaggingRegressor(rfr, n_estimators=100, max_samples=0.1, random_state=25)
parameters = {'n_estimators': [1000], 'max_depth': [6]}


parameters = {'nthread':[-1], #when use hyperthread, xgboost may become slower
              'objective':['reg:linear'],
              'learning_rate': [0.125,0.0125,0.05,0.075,0.1,0.15], #so called `eta` value
...
model = grid_search.GridSearchCV(estimator =gbr, param_grid = parameters, n_jobs = -1, cv = 5, verbose = 20, scoring='mean_squared_error')
model.fit(X, Y)
...
sample["relevance"] = predictions  

sample.to_csv('1gramdepth7learningrate07all.csv', index=False)

##dictionary downloaded using spell check
